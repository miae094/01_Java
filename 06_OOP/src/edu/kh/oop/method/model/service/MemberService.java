...
public class MemberService {
	...
	private Member memberInfo = null;	// null : 참조하는 값이 없음
										// 가입한 회원의 정보를 전역적으로 저장할 변수
	...
	public String login() {
		...
		if(memberId.equals(memberInfo.getMemberId()) &&
				memberPw.equals(memberInfo.getMemgerPw())) {
	...
	// -> 조회하려면?
	// -> 로그인을 했는지 안했는지 확인
	// 1. 로그인 했으면? 현재 로그인 한 사람의 정보를 저장하고 있는 객체에서 정보 얻어오기
	// (이름, 아이디, 나이)
	// 2. 로그인을 안했으면? 로그인 후 이용해주세요.
	public String selectMember() {
		System.out.println("\n*************회원 정보 조회***************");
		
		if(loginMember==null) {	// 로그인을 안했을 경우
			return "로그인 후 이용해주세요";
		}
	...
	public int updateMember() {
		...
		if(inputPw.equals(loginMember.getMemgerPw())) {
