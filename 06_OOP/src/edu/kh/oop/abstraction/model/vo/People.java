...
// model - 프로그램 로직과 관련되어 있는 데이터 장소를 저장하는 용도, 비즈니스 로직 
// VO - 값 저장용 객체를 만들기 위한 클래스를 모아두는 패키지
public class People {	// 국민(사람) 클래스
	...
	// 속성 == 값 == 데이터
	// 값을 저장하기 위한 변수 선언
	// -> 국민이라면 공통적으로 가지고 있는 속상만 작성(추상화)
	// 이름, 성별, 주민번호, 주소, 나이, 전화번호
	// 필드 == 필드변수 == 멤버변수
	
	// 캡슐화
	// - 데이터(속성)와 기능을 하나로 묶어서 관리하는 기법
	// - 데이터의 직접적인 제한하는것이 원칙이다.
	// -> 직접 접근을 못하기 때문에
	// 		클래스 내부에서 간접 접근 방법을 제공하는
	//		기능(메서드) 작성해둔다.
	// -> getter/ setter
	...
	// [접근제한자] 자료형 변수명;
	private String name;
	private char gender;		// 기본은 0이지만 유니코드 문자열로 바뀌면서 공백으로 나옴
	...
	*/
	...
	// getter/setter 자동생성 : art + shift + s 또는 상단의 Source 
	...
	public void setAge(int age) {
		// 메서드(기능) 형태은 getter/setter를 이용하면 본인이 원하는 기능을 추가할수있음
		// age가 0보다 작을 때 예외 발생시키기
		// age가 0보다 클 때 속성에 전달받은 값 세팅하기
		if(age>0) {
			this.age = age;	
		} else {
			throw new IllegalArgumentException("음수 안돼 돌아가");
