...
public class User {
	...
	// 매개변수 생성자 자동 완성
	// art + shift +s Generate Constructor using Fields..-> generate
	...
	// [오버로딩 조건]
	// 1) 메서드의 이름이 동일
	// 2) 매개변수의 개수, 타입(자료형), 순서 중 한개라도 달라야함.
	...
	public User(int Age) {}			// 매개변수의 개수는 같지만 타입이 다름
	// -> 오버로딩 성립
	
	public User(String usrId, int userAge) {}
	...
	// @Override : 메서드가 부모클래스의 메서드를 "정확히" 재정의(오버라이딩)하고있는지 검사
	// 1. 만약에 오타가 있거나, 부모클래스에 없는 메서드를 잘못 작성했을 경우 컴파일 오류 발생
	// 2. 이 메서드가 부모클래스를 재정의 했다는것을 한눈에 알 수 있음.
