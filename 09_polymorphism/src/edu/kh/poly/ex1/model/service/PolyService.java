...
//    자식타입 참조변수 = 자식객체주소
		...
		// 부모타입 참조변수 = 자식객체
		Car car2 = new Tesla();
		// 원래 대입연산이라 같은 자료형끼리만 되어야 정상인데 다형성 적용돼서 가능함.
		// Tesla 객체를 참조하는 변수의 타입이 Car(부모)이기 때문에
		// Tesla 객체가 Car(부모타입) 객체로 변화함
		...
		// The method discountOffer(double) is undefined for the type Car
		
		// -> car2, car3는 부모타입 참조변수(Car)이기 때문에
		// Tesla, Spark와 같은 자식객체의 고유한 필드, 메서드 사용 불가능
	...
	// 전달받은 Car또는 자식객체(Tesla, Spark)의 엔진, 연료, 바퀴갯수를 출력하는 메서드
	public void printCar(Car temp) {
		...
		// 부모타입 참조변수 = 자식타입객체
		// -> 다형성의 업캐스팅 모양과 똑같다
		// -> temp 라는 Car 타입 매개변수에는 Car, Tesla, Spark 의 주소가 참조되어도 된다.
	...
	public void ex2() {
		...
		System.out.println(arr[1] instanceof Spark);	// False (Spark와 Tesla는 관계없음)
	...
	public void ex3() {
		...
		// 그래서 형변환 하는 부분을 () 로 한번 묶어주고 나서 . 연산을 진행해야
	...
	public void ex4() {
		...
		// 말이 안되는건데 컴파일 에러가 안나네
		//Spark s1 = (Spark)c1;
		// 다운 캐스팅 잘못해도 컴파일 에러 발생 안함
		// -> 실행 시 발생(런타임 에러)
		// java.lang.ClassCastException : 형변한 예외
	...
	public void ex5() {
		...
		// 프로그램 "실행 전"
		// 컴파일러는 toString() 메서드가 Car에 있는걸로 인색해서
		...
		// 실행해보면 자식(Spark)의 toString() 메서드가 호출되는것을 확인
		// 경차엔진 / 휘발유 / 4 / 0.5
