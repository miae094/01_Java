...
//		i 개수 : 2
		...
		// 반복문을 통해 input.CharAt(i) 값을 arr[i]에 담는 과정
		for(int i = 0; i < arr.length; i++) {
		...
		System.out.print("문자 : ");
		// char 형으로 받을 수 없기 때문에 next 혹은 nextLine 선택 후 charAt 선택
		...
		// 결과 출력
		System.out.println(input + "에 " + ch +"가 존재하는 위치(인덱스) : " + indexNum);
...
//		1, 2, 3, 2, 1
		...
		int[] arr = new int[input];
		int indexNum = (arr.length/2);	// 중간 값
		for(int i = 0; i < arr.length; i++ ) {
			if(i <= indexNum) {
				arr[i] = i+1;
...
//		문자 개수 : 8
		...
		for(int i = 0; i<arr.length; i++) {
			...
			boolean flag = false;	// 중복체크용 flag 변수
			...
			if(!flag) {	// 중복이 아닐때
				...
				// 강사님 방법
				if(i==0) {	// 첫번째 글자
					System.out.print(arr[i]);	// 첫 글자는 무조건 입력되기 때문에
				}else {
					System.out.print(", " + arr[i]);
				}
				
/////////////////// 이렇게 하면 안됨 /////////////////////////
///끝 두글자가 같은 경우 마지막에 쉼표 찍힘				
//				if(i == (arr.length-1)) {	// 마지막 배열인지 확인
//					System.out.print(arr[i]);
...
//		[자바의 정석, 알고리즘, C프로그래밍, 인간관계, 자기계발, 영단어600]
		...
		while(!flag) {
			...
			if(str.equals("y")) {
				...
				num = sc.nextInt();
				sc.nextLine();	// 공백 처리
				arr2 = new String[arr.length+num];
				...
				// 원본에 얕은복사 하기(주소만) !!!!!!!!!! <- 이부분 중요해!!!!
				// 배열 크키가 계속 늘어나기 때문에 해줘야함
				arr = arr2;
			} else {
