...
public class StudentService {
	...
	// new ArrayList() : 기본생성자 -> 기본 크리 10짜리 리스트 생성
	...
	public StudentService() {	// 기본생성자
		...
		studentList.add(new Student("강하지", 30, "서울시 강남구", 'F', 80));
		studentList.add(new Student("오미나", 27, "충북 청주시", 'F', 90));
		studentList.add(new Student("김진영", 24, "서울시 서대문구", 'M', 70));
	...
	public void ex() { 		// List 테스트용 예제
		...
		// testList 에 꺼내온 인덱스 요소가 Student 혹은 상속관계인지 검사
	...
	*/
	...
	public void displayMenu() {
		...
		do {
			...
			try {
				...
				menuNum  = -1; // 임의값으로 바꿈
				// 첫 반복시 잘못 입력하는 경우 menuNum 이 초기값인 0을 가지고 있어
				// 반복문이 종료되는데,
				// 이를 방지하기 위해 임의 값 -1 입력
	...
	public String addStudent() throws InputMismatchException {
		...
		// throws InputMismatchException 써도되고 안써도됨
...
//		
		...
		// 향상된 for문은 인덱스 역할을 하는 변수가 없기때문에 따로 만들어서 사용해야함
		int index = 0;
	...
	public String updateStudent() {
		...
		if(studentList.isEmpty()) {
			...
			// temp는 수정 전 정보!!!!! set을 쓰면 새로 셋팅되면서 기존 정보를 뱉어낸대! 신기
	...
	public String removeStudent() {
		...
		if( str == 'Y') {
			...
			return temp.getName() + "님의 정보가 삭제되었습니다.";
		} else if(str == 'N') {
			return "취소";
		} else {
			return "올바른 형식의 문자를 입력하세요";
		}
...
boolean flag = true;
		...
		for(Student std : studentList) {
			...
			// boolean String.contains(문자열) : String 에 문자열이 포함되어 있으면 true/false
	 ...
	 * 익명 내부 클래스 : 이름이 없는 클래스를 즉석에서 선언해서 한번만 사용할 목적으로 사용.
	 * 객체를 생성하면서 바로 구현내용을 정희할 수 있음.
	 * 익명 내부 클래스 장점
	...
	public void sortByAge() {
		...
		/*	
		 * 
		 * 내가 정의 한 클래스를 내가 원하는 식으로 정렬을 해야하는 경우라면
	 	오버라이딩 필수임.
	 	이미 Integer나 String으로 만들어진 객체라면 따로 오버라이딩 필요없음
		이유:
		Collections.sort()는 **List**를 정렬하기 위해 각 객체를 비교해야 합니다.
		그때, Java에서 객체 비교를 어떻게 할지를 결정하는 방법은
		Comparable 인터페이스에 구현된 compareTo() 메서드에 의해 정의됩니다.
		만약 compareTo()를 오버라이드하지 않으면,
		Collections.sort()는 객체 간의 비교 방법을 알 수 없기 때문에 정렬할 수 없습니다.
		 */		
	...
	public void sortByName() {
		...
		Collections.sort(studentList, new Comparator<Student>() {
			...
			public int compare(Student o1, Student o2) {
				...
				// String.compareTo() : 자바에서 객체를 비교할 때 사용하는 메서드
				// (String 이 Comparable 을 상속받아 재정의 해둔 compareTo()메서드를 이용하는것)
				
				// compareTo() : 두 객체를 비교하고 순서를 결정
				// 반환값 : 0(같음), 양수(왼쪽 객체가 큼), 음수(왼쪽 객체가 큼)
